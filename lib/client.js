...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var UsageApi = (function () {
      ...
      UsageApi.prototype.opencodeGo = function (opts) {
        var search = new URLSearchParams()
        if (opts.fresh) search.set('fresh', '1')
        var q = search.toString()
        return fetch('/api/dsh-usage-stats/opencode-go' + (q ? '?' + q : ''), { headers: { accept: 'application/json' } }).then(readJson)
      }
      return UsageApi
    })()
    ...
    var styles = [
      '.dshus-page { font-family: var(--dsw-font-family, ui-sans-serif, system-ui, sans-serif); color: var(--dsw-alias-label-primary, #1f2430); box-sizing: border-box; --dshus-heat: #22c55e; }',
      ...
      /* metric cards */
      '.dshus-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; }',
      ...
      '.dshus-card .dshus-oc-wsub { font-size: 10.5px; color: var(--dsw-alias-label-caption, #9ca3af); margin-top: 3px; font-variant-numeric: tabular-nums; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }',
      ...
      '.dshus-go-head .k { margin-top: 0; font-size: 12px; }',
      '.dshus-go-pct { font-size: 22px; font-weight: 700; font-variant-numeric: tabular-nums; letter-spacing: -0.01em; line-height: 1; }',
      '.dshus-go-track { height: 8px; border-radius: 999px; background: var(--dsw-alias-bg-layer-2, #f2f3f6); overflow: hidden; }',
      '.dshus-go-fill { height: 100%; border-radius: 999px; transition: width 0.35s ease; }',
      '.dshus-go-foot { display: flex; justify-content: space-between; align-items: baseline; gap: 8px; margin-top: 9px; font-size: 11px; color: var(--dsw-alias-label-tertiary, #6b7280); font-variant-numeric: tabular-nums; white-space: nowrap; overflow: hidden; }',
      '.dshus-go-reset { color: var(--dsw-alias-label-caption, #9ca3af); font-size: 10.5px; }',
      ...
      '.dshus-module h3 { margin: 0 0 12px; font-size: 13px; font-weight: 600; letter-spacing: 0.02em; color: var(--dsw-alias-label-secondary, #4b5563); display: flex; align-items: center; gap: 8px; }',
      '.dshus-module h3 .dshus-count { font-size: 11px; font-weight: 400; color: var(--dsw-alias-label-caption, #9ca3af); }',

      /* heatmap — original GitHub-style calendar as ONE row of fixed-size
         cells (16px); the six-month window fits the pane width without
         horizontal scrolling; the strip is centred inside the card */
      '.dshus-heat-wrap { width: 100%; padding-bottom: 4px; display: flex; flex-direction: column; align-items: center; }',
      ...
      '.dshus-cell { width: 16px; height: 16px; border-radius: 3px; background: color-mix(in srgb, var(--dshus-heat, #22c55e) 12%, transparent); }',
      '.dshus-cell:hover { outline: 1px solid var(--dsw-alias-label-secondary, #4b5563); outline-offset: 1px; }',
      '.dshus-legend { display: flex; align-items: center; justify-content: center; gap: 5px; margin-top: 10px; font-size: 11px; color: var(--dsw-alias-label-secondary, #4b5563); }',
      ...
      /* opencode-go panel */
      '.dshus-oc-head { display: flex; align-items: center; justify-content: space-between; gap: 10px; margin-bottom: 12px; flex-wrap: wrap; }',
      '.dshus-oc-src { color: var(--dsw-alias-label-caption, #9ca3af); font-size: 11px; line-height: 16px; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 70%; }',
    ].join('\n')
      ...
      var weekCols = heat.weeks.map(function (week, w) {
        return React.createElement('div', { className: 'dshus-heat-col', key: week.monday },
          week.cells.map(function (cell, c) {
            ...
            // the default empty-cell green so the six-month strip stays one
      ...
      var bars = hourModels.map(function (item) {
        ...
        return React.createElement('div', {
          ...
          onMouseMove: function (e) {
            // Segment hovers show their own per-model tooltip (which bubbles
            // up here afterwards); only render the column-level breakdown
            // when the bar itself is hovered (the empty area above segments).
            if (e.target !== e.currentTarget) return
            var lines = ms.map(function (mm) {
              return (mm.provider ? mm.provider + ' · ' : '') + mm.model + '  ' + fmt(mm.billed) + ' tokens'
            })
            showTip(e, item.hour + '时', fmt(tot) + ' tokens' + (lines.length ? '\n' + lines.join('\n') : ''))
          },
          onMouseLeave: hideTip,
    ...
    /* ------------------------------------------------------------------ */
    /* opencode-go usage panel                                             */
    /* ------------------------------------------------------------------ */

    /**
     * Panel at the bottom of the 用量统计 page: reads the official OpenCode Go
     * subscription quota (opencode.ai/zen/go/v1/usage) through the host route
     * and shows the three rolling quota windows (5 hours / 7 days / 30 days)
     * as opencode.ai-style progress-bar cards. Rendered purely from the
     * official quota — the local opencode database is never used. When no
     * official quota is available the whole section is not rendered at all.
     */
    function OpencodePanel(props) {
      var api = props.api

      // All hooks are declared unconditionally at the top (before the early
      // return below) so React's hook order never breaks.
      var goState = React.useState(null)
      var goData = goState[0]
      var setGoData = goState[1]
      var goLoadingState = React.useState(false)
      var goLoading = goLoadingState[0]
      var setGoLoading = goLoadingState[1]
      ...
      function load(fresh) {
        setGoLoading(true)
        api.opencodeGo({ fresh: fresh })
          .then(function (body) {
            setGoData(body)
            setGoLoading(false)
          })
          .catch(function (err) {
            setGoData({ ok: false, available: false, reason: 'error', error: String(err && err.message || err) })
            setGoLoading(false)
          })
      }
      ...
      var refreshBtn = React.createElement('button', {
        className: 'dshus-btn',
        onClick: function () { load(true) },
        disabled: goLoading,
        title: '重新读取 OpenCode Go 官方配额',
      }, goLoading ? React.createElement('span', { className: 'dshus-spin' }) : React.createElement('span', null, '刷新'))

      // The whole section renders only when the official OpenCode Go quota is
      // available: no login, endpoint failure or in-flight loading → nothing
      // is displayed (no local library fallback, no placeholder text).
      if (!(goData !== null && goData.ok === true && goData.available === true)) {
        return null
      }

      // Custom hover tooltip (same style as the other charts).
      var showTip = function (e, name, val) {
        var x = e.clientX + 14
        var y = e.clientY + 14
        if (x + 280 > window.innerWidth) x = e.clientX - 280
      ...
      // The three rolling quota windows — official OpenCode Go subscription
      // quota only (progress-bar cards like opencode.ai/zen/go: percent used,
      // USD spent vs window limit, reset countdown).
      var GO_WINDOW_ORDER = [
        { key: 'rolling', title: '5小时用量' },
        { key: 'weekly', title: '周用量' },
        { key: 'monthly', title: '月用量' },
      ]
      var goWindows = goData.windows || {}
      var windowCards = GO_WINDOW_ORDER.map(function (wd, idx) {
        var gw = goWindows[wd.key] || null
        if (gw === null) return null
        // Official quota card: big percent + progress bar + $ used / limit
        // + reset countdown.
        var pct = Math.max(0, Math.min(100, Number(gw.percent) || 0))
        var usedUsd = (pct / 100) * (Number(gw.limit) || 0)
        var barColor = pct >= 90 ? '#ef4444' : pct >= 70 ? '#f59e0b' : '#22c55e'
        var glines = []
        glines.push('官方配额：已用 ' + pct + '%')
        if (gw.limit) glines.push('已用 $' + usedUsd.toFixed(2) + ' / $' + gw.limit)
        if (gw.resetsAt) glines.push('重置倒计时：' + fmtReset(gw.resetsAt))
        return React.createElement('div', {
          key: wd.key,
          className: 'dshus-card dshus-go-card' + (idx === 2 ? ' dshus-go-last' : ''),
          onMouseMove: function (e) { showTip(e, wd.title + ' · ' + pct + '%', glines.join('\n')) },
          onMouseLeave: hideTip,
        },
          React.createElement('div', { className: 'dshus-go-head' },
            React.createElement('div', { className: 'k' }, wd.title),
            React.createElement('div', { className: 'dshus-go-pct', style: { color: barColor } }, pct + '%'),
          ),
          React.createElement('div', { className: 'dshus-go-track' },
            React.createElement('div', { className: 'dshus-go-fill', style: { width: pct + '%', background: barColor } }),
          ),
          React.createElement('div', { className: 'dshus-go-foot' },
            React.createElement('span', null, gw.limit ? '$' + usedUsd.toFixed(2) + ' / $' + gw.limit : ''),
            React.createElement('span', { className: 'dshus-go-reset' }, gw.resetsAt ? fmtReset(gw.resetsAt) + '重置' : ''),
          ),
        )
      })

      var goSyncedAt = goData.syncedAt ? new Date(goData.syncedAt) : null
      var goSyncedLabel = goSyncedAt
        ? (goSyncedAt.getMonth() + 1) + '/' + goSyncedAt.getDate() + ' ' + String(goSyncedAt.getHours()).padStart(2, '0') + ':' + String(goSyncedAt.getMinutes()).padStart(2, '0')
        : ''

      return React.createElement('div', { className: 'dshus-module' },
        React.createElement('h3', null,
          'opencode-go 使用情况',
          React.createElement('span', { className: 'dshus-count' }, '（OpenCode Go 官方配额 · 5 小时 / 周 / 月）'),
        ),
        React.createElement('div', { className: 'dshus-oc-head' },
          React.createElement('div', { className: 'dshus-oc-src', title: 'opencode.ai/zen/go/v1/usage' },
            '数据源：OpenCode Go 官方配额' + (goSyncedLabel ? ' · 更新于 ' + goSyncedLabel : '')),
          refreshBtn,
        ),
        React.createElement('div', { className: 'dshus-go-cards' }, windowCards),
        tipEl,
      )
    }
    ...
    function UsageStatsSection(props) {
        ...
        cards = React.createElement('div', { className: 'dshus-cards' },
          ...
          topModel
            ? React.createElement('div', { className: 'dshus-card dshus-card-model' },
                ...
                React.createElement('div', { className: 'sub' }, topModel.provider + ' · ' + (totals.billed > 0 ? formatPct((topModel.billed / totals.billed) * 100) : '')),
      ...
      return React.createElement('div', { className: 'dshus-page' },
        ...
        // opencode-go 套餐用量: rendered purely from the official OpenCode Go
        // quota; the whole section is absent when no quota is available.
        React.createElement(OpencodePanel, { api: api }),
      )
    }
